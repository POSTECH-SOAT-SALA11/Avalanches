...
public class ProdutoApi implements ProdutoApiInterface {
    ...
    @Autowired
    private ProdutoUseCaseInterface produtoUseCaseInterface;

    @PostMapping
    @Override
    public ResponseEntity<Void> cadastrar(@Valid @RequestBody ProdutoRequest produto) {
        produtoUseCaseInterface.cadastrarProduto(Convert.produtoRequestToProduto(produto));
    ...
    public ResponseEntity<Void> atualizar(@PathVariable int id, @Valid @RequestBody ProdutoRequest produto) {
        ...
        produtoEntity.id = id;
        produtoUseCaseInterface.atualizarProduto(produtoEntity);
    ...
    public ResponseEntity<Void> excluir(@PathVariable int id) {
        produtoUseCaseInterface.excluirProduto(id);
        return ResponseEntity.status(HttpStatus.OK).build();
    }
    ...
    public ResponseEntity<List<ProdutoResponse>> consultarPorCategoria(@PathVariable("categoriaProduto") CategoriaProduto categoriaProduto){
        var response = Convert.listProdutoToListProdutoResponse(produtoUseCaseInterface.consultarProdutos(categoriaProduto));
